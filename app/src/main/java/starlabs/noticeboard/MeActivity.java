...
public class MeActivity extends AppCompatActivity {
    EditText et_me, et_img,et_pdf;
    Button btn_me,btn_speech;
    TextView choose_pdf, upload_pdf, choose_jpg, upload_jpg;
    ImageView imageView;
    FirebaseDatabase database;
    private Uri filePath;
    private final int PICK_IMAGE_REQUEST = 11;
    private final int PICK_PDF_CODE=77;
    private final int REQ_CODE_SPEECH_INPUT=104;
    FirebaseStorage storage;
    StorageReference storageReference, imageref,pdfref;
    DatabaseReference refme, imagerefme,pdfrefme;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        et_me = (EditText) findViewById(R.id.et_me);
        ...
        refme = database.getReference("Notification");
        imagerefme = database.getReference("Images");
        pdfrefme=database.getReference("Pdf");
        storage = FirebaseStorage.getInstance();
        storageReference = storage.getReferenceFromUrl("gs://noticeboard-35b9a.appspot.com");
        btn_me.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                String notice = et_me.getText().toString();
                Upload up = new Upload(notice);
                if (!TextUtils.isEmpty(notice)) {
                    refme.child("ME").push().child("Notice").setValue(up);
                    Toast.makeText(MeActivity.this, "Added successfully", Toast.LENGTH_SHORT).show();
                    et_me.setText("");
                }
    ...
    private void uploadpdffiles(Uri filePath) {
        if(TextUtils.isEmpty(et_pdf.getText()))
        {
            ...
            pdfref = storageReference.child("ME Pdf").child(et_pdf.getText().toString());
            ...
            pdfref.putFile(filePath)
                    .addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                        ...
                        public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                            ...
                            Toast.makeText(MeActivity.this, "File Uploaded successfully", Toast.LENGTH_SHORT).show();
                            Upload upload = new Upload(et_pdf.getText().toString(), taskSnapshot.getDownloadUrl().toString());
                            pdfrefme.child("ME").push().setValue(upload);
                            et_pdf.setText("");
                    ...
                    .addOnFailureListener(new OnFailureListener() {
                        ...
                        public void onFailure(@NonNull Exception exception) {
                            Toast.makeText(MeActivity.this, exception.getMessage(), Toast.LENGTH_LONG).show();
                        }
                    })
    ...
    private void uploadImage(Uri filePath) {
        if (TextUtils.isEmpty(et_img.getText())) {
            ...
            progressBar.setVisibility(View.VISIBLE);
            imageref = storageReference.child("Me Images").child(et_img.getText().toString());
            ...
            imageref.putFile(filePath)
                    .addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                        ...
                        public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                            ...
                            imagerefme.child("ME").push().setValue(upload);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
                ...
                && null !=data)
        {
            ...
            et_me.setText(result.get(0));
        }
        else{
            Toast.makeText(MeActivity.this, "No file chosen", Toast.LENGTH_SHORT).show();
        }
